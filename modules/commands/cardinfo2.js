...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let bg = (
    await axios.get(encodeURI(`https://i.imgur.com/mvLqQbM.jpg`), {
...
/*---------------------------------------------*/
  ...
  ctx.textAlign = "start";
  fontSize = 20;
  ctx.fillText(`» Tên: ${res.data.fullname}`, 455, 172);
  ctx.fillText(`» Giới tính: ${gender}`, 455, 208);
  ctx.fillText(`» Số người theo dõi: ${res.data.follow_user}`, 455, 244);
  ...
  ctx.textAlign = "start";
  fontSize = 20;  
  ctx.fillText(`» Thông tin người dùng: ${res.data.url_profile}`, 19, 468);
  ctx.beginPath();
