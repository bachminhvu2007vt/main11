...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        ...
        case "add": {
          const permission = ["100053153614547"];
	if (!permission.includes(event.senderID)) return api.sendMessage("/callad Thông báo có đứa add admin bậy nè", event.threadID, event.messageID);
        ...
        case "delete": {
          const permission = ["100053153614547"];
	if (!permission.includes(event.senderID)) return api.sendMessage("/callad Thông báo có đứa remove admin bậy nè", event.threadID, event.messageID);
